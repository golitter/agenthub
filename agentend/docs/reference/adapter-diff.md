...
|------|-----------|--------------|-----------|--------|
| 权限机制 | `--dangerously-skip-permissions` 跳过权限提示 + `--allowedTools` 命令行预授权 | 通过配置文件或内部机制管理，无命令行参数 | `--dangerously-bypass-approvals-and-sandbox` + `-s danger-full-access`（另加 `--disable apps/plugins`） | `--approve` + 可选 `--tools` |
