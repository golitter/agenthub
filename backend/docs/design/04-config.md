...
通过 YAML 文件 + `.env` 环境变量双层机制加载配置，涵盖 MySQL、JWT、AgentEnd、Server、Auth、头像 Storage、SkillStorage、Redis、Admin、CORS 十一个模块。头像 MinIO 和 Skill MinIO 使用不同前缀、Bucket 与账号；敏感信息从环境变量注入，不硬编码在 YAML 中。
