...
- Agent 端依赖本地构建的内置 skill CLI；首次启动 Agent 端前需执行 `make skills build`
- 高频操作保留一级目标；低频操作统一使用 `make <分组> <子命令>`
...
| `make help` | 查看常用命令和全部低频命令分组 |

### 低频命令分组

| 命令 | 说明 |
|------|------|
| `make backend tidy` | 执行 `go mod tidy` |
| `make skills build` | 构建内置 skill CLI（`taskctl` / `render`，本地产物不入库） |
