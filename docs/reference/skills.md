...
WorkspaceManager.create()
  → SkillProvisioner.provision(worktree_path, agent_type)
    → 复制 builtin skills 到 <config_dir>/skills/（如 .claude/skills/、.opencode/skills/、.codex/skills/、.pi/skills/，config_dir 由 agentend/agents.json 决定）
...
> 当前内置两个 Skill：`taskctl`（任务协作）与 `render`（卡片渲染），均为 Go 编译 CLI，源码位于 `agentend/src/skills/builtin/<name>/`（含 `main.go` + `go.mod`）。可执行文件通过 `make skills build` 按当前平台生成，属于本地产物，不入库。
