...
聊天主容器（Smart 组件），纵向三段式布局：Header（`min-h-14`）、消息区、输入区。核心 hook `useChatStream` 返回 `{ state, sendMessage, stopRun, isCancelling, canStop, historyError, retryHistory }`（群聊场景透传 `includeTaskMessages` 等选项）：
...
export function ChatArea({ taskId, sessionId, agentType = AGENT_TYPES.ClaudeCode, agentName, avatarUrl, repoPath, isGroupChat, ... }: ChatAreaProps) {
  const { state, sendMessage, stopRun, isCancelling, canStop, historyError, retryHistory } = useChatStream(taskId, sessionId, agentType, {
