...
新建对话时，如果用户输入的目录路径存在但不是 Git 仓库，支持通过确认目录名后自动执行 `git init`，无需手动操作终端。前端 `RepoPathInput` 组件检测 "不是 git 仓库" 错误后进入确认态，用户输入目录名匹配后调用 `initGitRepo` API 完成初始化。
...
**文件**: `frontend/src/lib/api.ts`（`initGitRepo` 函数，约 line 408）
